...
@implementation UILabel (HCYLabelTapAction)
...
{
    NSArray *strArray = [NSArray arrayWithObject:keyString];
    [self getRangesWithStrings:strArray];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                TestModel *model = self.attributeStrings[j];
